# European ash pangenome scripts
### Scripts associated with ash pangenome publication - preprint https://doi.org/10.1101/2025.07.16.665114
### Subfolders: 

#### 1_GenomeQuality - Quality assesment of the phased genome assemblies from Cantata Bio (BATG-1.0 and haplotype 2)
#### 2_Annotation - Annotation of BATG-1.0 using BRAKER3
#### 3_ONT_TrimMapSNP - Scripts for QCing/Trimming ONT data, mapping it to BATG-1.0, calling SNPs and producing a PCA for these
#### 4_SVCalling - calling SVs from the ONT data
#### 5_CharacteriseSVs - characterising frequency, lengths of SVs
#### 6_constructpangenome - constructing pangenome using vg:
#### 7_PangenomeAnnotation - annotating the pangenome, explictly using SVs included in the pangenome
#### 8_Construction_InSilicoPool  - constructing of the pangenome using vg; assessing quality of short-read mapping of subset of Stocks et al. data. Also constructs an in silico poolseq dataset, compares the allele frequency estimates using this to those estimated using the individuals used to construct the in silico pool

####Note 1: Scripts containing ###ARRAY_BIT### are ran using a custom gridengine submission script (usage qarray.sh your_script.sh), see https://github.com/danielwood1992/qarray_gridengine_slurm for details 





