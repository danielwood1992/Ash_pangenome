...
outdir="/data/SBCS-BuggsLab-Ash/DanielWood/PG2_PanGenome/PG2_12_merge/NC_4_4_3.1_sensitivity";

overall_results="$outdir/NC_4_4_3.1_results.thresholds.txt";

truncate -s 0 $outdir/NC_4_4_3.1_results.thresholds.txt;
...
comparison_function_septype(){
	...
	for type in ${my_types[@]};
			...
			do echo $type;
			grep "#\|SVTYPE=$type" $vcf1 > $vcf1.$distance.$type;
			grep "#\|SVTYPE=$type" $vcf2 > $vcf2.$distance.$type;

			#ii) Merge with SURVIVOR
			truncate -s 0 $outdir/$base_name.${results}.$name.$type.tmp;
			echo $vcf1.$distance.$type >>  $outdir/$base_name.${results}.$name.$type.tmp;
			echo $vcf2.$distance.$type >>  $outdir/$base_name.${results}.$name.$type.tmp;
			$SURVIVOR merge $outdir/$base_name.${results}.$name.$type.tmp $distance 2 1 1 1 50 $outdir/$base_name.${results}.$name.$type.vcf;
...
#So this is doing the OR merging, which I guess comes from the svim-asm stuff
comparison_function_septype_OR(){
	...
	for type in ${my_types[@]};
			...
			echo $vcf1.$distance.$type >>  $outdir/$base_name.${results}.$name.$type.tmp;
			echo $vcf2.$distance.$type >>  $outdir/$base_name.${results}.$name.$type.tmp;
			$SURVIVOR merge $outdir/$base_name.${results}.$name.$type.tmp $distance 1 1 1 1 50 $outdir/$base_name.${results}.$name.$type.vcf;
...
threshold_array=(1 10 25 50 100 150 200 300 500 1000 2000);

for threshold in "${threshold_array[@]}"
	do echo $threshold;
	results="NC_4_4_3.1_results.$threshold.txt";
	...
	comparison_function_septype $sniffles $cutesv "1_AND_2_bothreadmapping" $threshold;
	comparison_function_septype_OR $outdir/$base_name.$results."1_AND_2_bothreadmapping".all_types.vcf $svim "1_AND_2_OR_4" $threshold;
	num=$(grep -v '#' $outdir/$base_name.${results}.$name.all_types.vcf | wc -l)
	echo $base_name $distance $num >> $overall_results
