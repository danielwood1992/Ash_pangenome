...
#Load modules
outdir="/data/SBCS-BuggsLab-Ash/DanielWood/PG2_PanGenome/PG2_12_merge/NC_4_4_4.1_sensitivity";

SURVIVOR="/data/home/mpx545/SURVIVOR/Debug/SURVIVOR";

overall_results="NC_4_4_4.4_indmerge_sensitivity.txt";

truncate -s 0 $outdir/$overall_results;
...
cat $name_list | sed 's/^.*\///g' | sed 's/\..*//g' | sed 's/$/.PG2_12_1_results.txt.1_AND_2_OR_4.all_types.vcf/g' | sed 's/^/\/data\/SBCS-BuggsLab-Ash\/DanielWood\/PG2_PanGenome\/PG2_12_merge\//g' > $name_list.AND.vcfs;
...
#
		...
		$SURVIVOR merge $outdir/$results.$type.vcf_list $threshold 1 1 1 1 50 $outdir/$results.$type.vcf;
	done;
	...
	num=$(grep -v '#' $outdir/$results.all_types.tags.vcf | wc -l);
	echo $threshold $num >> $outdir/$overall_results;
done;
